...
async function standardizeConvoObject(convo) {
	try {
		...
		let ownerInfo = await db.users.findOne({
			raw: true,
			where: {
				id: ownerId,
			},
		});
		let participantsInfo = await db.users.findAll({
			raw: true,
			where: {
				id: participants
					.map((e) => e.userId)
		...
		participantsInfo = [ownerInfo, ...participantsInfo];
...
async function cascadeDeleteUser(userId) {
	try {
		...
		if (posts.length !== 0) {
			await Promise.all(
				posts.forEach(async (p) => await cascadeDeletePost(p.id))
			);
		}
